...
class AIROICalculator {
  ...
  init() {
    ...
    const displayMap = {
      'dev-cost': 'dev-cost-val',
      'maint-cost': 'maint-cost-val',
    ...
    inputs.forEach(id => {
      ...
      input.addEventListener('input', (e) => {
        ...
        const displayElement = document.getElementById(displayMap[id]);

        if (displayElement) {
          if (id === 'duration') {
            displayElement.textContent = val;
          } else {
            displayElement.textContent = val + 'K';
          }
        }
  ...
  calculate() {
    ...
    document.getElementById('total-benefits').textContent =
    ...
    if (netValue > benefits) {
      recommendation.textContent =
        'Recommendation: Strong investment - positive NPV';
      recommendation.style.background = '#d1fae5';
      recommendation.style.color = '#065f46';
    } else if (netValue > 0) {
      recommendation.textContent =
        'Recommendation: Consider timeline - break-even within project life';
      recommendation.style.background = '#fef3c7';
      recommendation.style.color = '#92400e';
    } else {
      recommendation.textContent =
        'Recommendation: Consider project carefully - negative NPV';
